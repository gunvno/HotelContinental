...
export function Select<TValue extends string = string>({
  ...
  return (
    <div ref={rootRef} className={cn("relative space-y-2", className)}>
      {label ? (
        <label className="text-xs font-bold tracking-[0.16em] text-[#7c6f63] uppercase">
          {label}
        </label>
      ) : null}
      <button
        ...
        className={cn(
          "flex h-12 w-full items-center justify-between gap-3 rounded-xl border border-[#decdb9] bg-white px-3 text-left text-sm font-semibold text-[#1f1a17] shadow-sm transition outline-none hover:border-[#c8792a] focus-visible:border-[#c8792a] focus-visible:ring-2 focus-visible:ring-[#c8792a]/20 disabled:cursor-not-allowed disabled:bg-[#f2eadf] disabled:text-[#9f8a77]",
          buttonClassName,
        )}
      >
