...
export function RoomFilter({ amenities, onApply }: RoomFilterProps) {
  ...
  const capacityLabel = capacity === 0 ? "Tất cả sức chứa" : `${capacity} người`;
  ...
  return (
    <aside className="border-border/60 bg-muted/70 space-y-6 rounded-[28px] border p-6 shadow-[0_18px_55px_-40px_rgba(31,41,55,0.35)] lg:sticky lg:top-28">
      <div>
        <p className="text-foreground text-2xl font-semibold whitespace-nowrap">
          Bộ lọc tìm kiếm
        </p>
      ...
      <div className="space-y-2">
        ...
        <div className="border-border bg-background text-muted-foreground flex items-center gap-3 rounded-xl border px-4 py-3 text-sm shadow-sm">
          ...
          <input
            ...
            value={minPrice}
            onChange={(event) =>
              setMinPrice(Number(event.target.value.replace(/[^0-9]/g, "")))
            }
        ...
        <label className="block space-y-2">
          ...
          <div ref={capacityDropdownRef} className="relative">
            <button
              ...
              className="border-border bg-background text-foreground hover:border-ring/70 focus:ring-ring/20 flex w-full items-center justify-between rounded-xl border px-4 py-3 text-sm shadow-sm transition focus:ring-2 focus:outline-none"
            >
                ...
                {capacityOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`hover:bg-muted/70 flex w-full items-center justify-between px-4 py-3 text-left text-sm transition first:rounded-t-2xl last:rounded-b-2xl ${active ? "bg-muted/70 text-foreground font-medium" : "text-foreground/90"}`}
                    >
                      <span>{option.label}</span>
                      {active ? (
                        <span className="text-ring text-xs">Đang chọn</span>
                      ) : null}
                    </button>
                  );
                })}
