...
export default function BuildingsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setError("Không tải được danh sách tòa nhà. Kiểm tra gateway, room-service và token ADMIN.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.buildingName.trim() || !form.defaultRoomTypeId || !form.defaultRoomSize.trim()) {
    ...
    if (!Number.isFinite(defaultPricePerDay) || defaultPricePerDay <= 0 || !Number.isFinite(defaultPricePerHour) || defaultPricePerHour <= 0) {
    ...
    try {
      ...
      setMessage(`Đã tạo ${result.createdFloorCount} tầng và ${result.createdRoomCount} phòng cho ${result.building.name}.`);
      setForm({ ...defaultFormState, defaultRoomTypeId: roomTypes[0]?.id || "" });
      await loadData();
    } catch (submitError) {
      console.error(submitError);
      setError("Không thể khởi tạo tòa nhà. Kiểm tra dữ liệu, token ADMIN và room-service.");
    } finally {
    ...
    <div className="space-y-7">
      <section className="relative overflow-hidden rounded-[2rem] border border-[#decdb9] bg-[#21170f] p-6 text-white shadow-[0_30px_80px_-52px_rgba(33,23,15,0.95)] lg:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.32em] text-[#e8c990]">Hotel structure</p>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-[#eadbc4]">
              Đây là nơi setup cấu trúc vật lý của khách sạn. Sau khi tạo tòa và tầng, trang Phòng sẽ chỉ dùng để quản lý từng phòng cụ thể.
            </p>
          </div>
          <Button
            type="button"
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <MetricCard label="Tổng tòa nhà" value={String(buildings.length)} icon={<Building2 className="h-5 w-5" />} />
        <MetricCard label="Tổng tầng" value={String(totalFloors)} icon={<Layers3 className="h-5 w-5" />} />
        <MetricCard label="Loại phòng mặc định" value={String(roomTypes.length)} icon={<Plus className="h-5 w-5" />} />
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <BuildingSetupForm
          ...
          onSubmit={onSubmit}
        />
        <BuildingList buildings={buildings} floorsByBuilding={floorsByBuilding} isLoading={isLoading} />
      </div>
    </div>
  );
}
  ...
  const previewRooms =
    Number.isFinite(floorStart) && Number.isFinite(roomsPerFloor) && roomsPerFloor > 0
      ? Array.from({ length: Math.min(roomsPerFloor, 6) }, (_, index) =>
          formatRoomNumber(form.roomNumberPattern || "{floor}{room:02}", floorStart, index + 1),
        )
      : [];

  return (
    <form onSubmit={onSubmit} className="rounded-[1.75rem] border border-[#decdb9] bg-white/78 p-6 shadow-sm">
      <p className="text-xs font-black uppercase tracking-[0.28em] text-[#9b5c24]">Khởi tạo ban đầu</p>
      <h3 className="mt-2 font-serif text-4xl font-bold">Tạo tòa, tầng và phòng hàng loạt</h3>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <Field label="Tên tòa nhà *">
          <Input value={form.buildingName} onChange={(event) => onChange("buildingName", event.target.value)} placeholder="Continental Tower A" />
        </Field>
        ...
        <Field label="Tầng bắt đầu *">
          <Input value={form.floorStart} onChange={(event) => onChange("floorStart", event.target.value.replace(/[^\d-]/g, ""))} inputMode="numeric" />
        </Field>
        <Field label="Tầng kết thúc *">
          <Input value={form.floorEnd} onChange={(event) => onChange("floorEnd", event.target.value.replace(/[^\d-]/g, ""))} inputMode="numeric" />
        </Field>
        <Field label="Số phòng mỗi tầng *">
          <Input value={form.roomsPerFloor} onChange={(event) => onChange("roomsPerFloor", event.target.value.replace(/[^\d]/g, ""))} inputMode="numeric" />
        </Field>
        <Field label="Mẫu số phòng *">
          <Input value={form.roomNumberPattern} onChange={(event) => onChange("roomNumberPattern", event.target.value)} placeholder="{floor}{room:02}" />
        </Field>
        <Field label="Giá ngày mặc định *">
          <Input value={form.defaultPricePerDay} onChange={(event) => onChange("defaultPricePerDay", event.target.value.replace(/[^\d]/g, ""))} inputMode="numeric" />
        </Field>
        <Field label="Giá giờ mặc định *">
          <Input value={form.defaultPricePerHour} onChange={(event) => onChange("defaultPricePerHour", event.target.value.replace(/[^\d]/g, ""))} inputMode="numeric" />
        </Field>
        <Field label="Diện tích mặc định *">
          <Input value={form.defaultRoomSize} onChange={(event) => onChange("defaultRoomSize", event.target.value)} placeholder="32m2" />
        </Field>
        <Field label="Phòng bỏ qua">
          <Input value={form.skipRoomNumbers} onChange={(event) => onChange("skipRoomNumbers", event.target.value)} placeholder="404, 413, 1414" />
        </Field>
        <div className="md:col-span-2">
          <Field label="Địa chỉ tòa nhà">
            <Input value={form.address} onChange={(event) => onChange("address", event.target.value)} placeholder="VD: 132 Đồng Khởi, Quận 1" />
          </Field>
        </div>
        <div className="md:col-span-2">
          <Field label="Mô tả">
            <Input value={form.description} onChange={(event) => onChange("description", event.target.value)} placeholder="Khu phòng chính" />
          </Field>
        </div>
      </div>
      <div className="mt-5 rounded-2xl border border-[#eadfcd] bg-[#fbf7ef] p-4">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#9b8c7d]">Preview mã phòng</p>
        <p className="mt-2 font-black">{previewRooms.length ? previewRooms.join(", ") : "Nhập số tầng và số phòng để xem trước."}</p>
      </div>
...
function BuildingList({
  ...
  return (
    <section className="rounded-[1.75rem] border border-[#decdb9] bg-white/78 p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-[#75695d]">Tòa nhà và tầng đang được lưu trong room-service.</p>
      <div className="mt-6 space-y-4">
        {isLoading ? <p className="rounded-2xl border border-[#eadfcd] bg-[#fffaf2] p-4 font-bold">Đang tải dữ liệu...</p> : null}
        ...
        {buildings.map((building) => {
          ...
          return (
            <article key={building.id} className="rounded-3xl border border-[#eadfcd] bg-[#fffaf2] p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h4 className="text-xl font-black">{building.name}</h4>
                  <p className="mt-1 text-sm text-[#75695d]">{building.description || "Chưa có mô tả"}</p>
                  <p className="mt-1 text-sm font-semibold text-[#5f5144]">{building.address || "Chưa có địa chỉ"}</p>
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {floors.map((floor) => (
                  <span key={floor.id} className="rounded-full border border-[#decdb9] bg-white px-3 py-1 text-xs font-bold text-[#5f5144]">
