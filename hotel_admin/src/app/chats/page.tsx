...
import { Archive, CheckCircle2, Circle, Inbox, Loader2, MessageCircle, RefreshCcw, Search, Send } from "lucide-react";
...
export default function ChatsPage() {
  ...
  const loadConversations = useCallback(async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      const sorted = [...data].sort((a, b) => getTime(b.lastMessageTime ?? b.createdTime) - getTime(a.lastMessageTime ?? a.createdTime));

      setConversations(sorted);
      setSelectedId((current) => {
        if (current && sorted.some((conversation) => conversation.id === current)) return current;
        return sorted.find((conversation) => conversation.status === "OPEN")?.id ?? sorted[0]?.id ?? null;
      });
      setError(null);
    } catch {
      setError("Không thể tải danh sách hội thoại. Hãy kiểm tra chat-service, api-gateway và quyền CHAT_STAFF_VIEW.");
    } finally {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      const customerText = `${displayCustomerName(conversation)} ${conversation.customerId}`.toLowerCase();
      const messageText = (conversation.lastMessage ?? "").toLowerCase();
      const matchesQuery = !normalizedQuery || customerText.includes(normalizedQuery) || messageText.includes(normalizedQuery);
  ...
  async function handleReply() {
    ...
    try {
      ...
      setError("Không thể gửi phản hồi. Hãy kiểm tra quyền CHAT_STAFF_REPLY hoặc trạng thái hội thoại.");
    } finally {
      setAction(null);
    }
  }
  ...
  if (!canView) {
    return <PermissionDenied message="Bạn không có quyền CHAT_STAFF_VIEW để xem tin nhắn hỗ trợ." />;
  }

  return (
    <div className="space-y-4">
      <section className="flex flex-col gap-4 rounded-2xl border border-[#decdb9] bg-white/85 p-5 shadow-sm dark:border-[#3a2e24] dark:bg-white/[0.06] md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#9b5c24]">Hỗ trợ khách hàng</p>
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-[#17213a] dark:text-[#f8f1e7]">Tin nhắn</h2>
        </div>
        <Button type="button" onClick={() => void handleRefresh()} disabled={loadingConversations || isBusy} className="gap-2">
          <RefreshCcw className={cn("h-4 w-4", action === "refresh" || loadingConversations ? "animate-spin" : "")} />
          Tải lại
        </Button>
      </section>
      ...
      <section className="flex min-h-[700px] flex-col overflow-hidden rounded-2xl border border-[#decdb9] bg-white shadow-sm dark:border-[#3a2e24] dark:bg-[#15110d] lg:flex-row">
        <aside className="flex shrink-0 flex-col border-b border-[#decdb9] bg-[#fbf8f2] dark:border-[#3a2e24] dark:bg-[#17130f] lg:w-[360px] lg:border-b-0 lg:border-r">
          <div className="space-y-3 border-b border-[#decdb9] p-4 dark:border-[#3a2e24]">
            ...
            <select
              value={status}
              onChange={(event) => setStatus(event.target.value as "ALL" | ChatConversationResponse["status"])}
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-[#17213a] dark:text-[#f8f1e7]">{displayCustomerName(conversation)}</p>
                  ...
                  <p className="mt-2 text-[11px] font-semibold text-[#9b5c24]">{formatDateTime(conversation.lastMessageTime ?? conversation.createdTime)}</p>
                </button>
              ))
            )}
          </div>
            ...
            <>
              <header className="flex items-center justify-between gap-4 border-b border-[#decdb9] px-5 py-4 dark:border-[#3a2e24]">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="truncate font-bold text-[#17213a] dark:text-[#f8f1e7]">{displayCustomerName(selectedConversation)}</h3>
                    <p className="truncate text-xs text-[#8a7967] dark:text-[#cbbba8]">ID: {shortId(selectedConversation.customerId)}</p>
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {selectedConversation.status === "OPEN" && canClose ? (
                    ...
                    >
                      {action === "close" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Archive className="h-3.5 w-3.5" />}
                      Kết thúc
                    </button>
                  ) : null}
                </div>
                ...
                {selectedConversation.status === "CLOSED" ? (
                  ...
                  <div className="flex items-end gap-3">
                    <textarea
                      ...
                      placeholder={canReply ? "Nhập phản hồi..." : "Bạn chưa có quyền trả lời"}
                      disabled={!canReply || isBusy}
                      rows={1}
                      className="min-h-12 flex-1 resize-none rounded-2xl border border-[#decdb9] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#c8792a] focus:ring-2 focus:ring-[#c8792a]/20 disabled:opacity-60 dark:border-[#3a2e24] dark:bg-[#211a14]"
                    />
                    <button
                      type="button"
                    ...
                    >
                      {action === "reply" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    </button>
                  </div>
                )}
              </footer>
...
function StatusPill({ status, compact = false }: { status: ChatConversationResponse["status"]; compact?: boolean }) {
  const open = status === "OPEN";

  return (
    <span
      className={cn(
        "inline-flex shrink-0 items-center gap-1 rounded-full font-black uppercase tracking-[0.12em]",
        compact ? "px-2 py-1 text-[10px]" : "px-2.5 py-1 text-[11px]",
        open ? "bg-emerald-50 text-emerald-700" : "bg-[#f1ece4] text-[#66584b]",
      )}
    >
      {open ? <Circle className="h-2.5 w-2.5 fill-current" /> : <CheckCircle2 className="h-3 w-3" />}
      {statusLabel[status]}
    </span>
  );
}
...
function MessageBubble({ message }: { message: ChatMessageResponse }) {
  ...
  return (
    <div className={cn("flex", fromCustomer || fromSystem ? "justify-start" : "justify-end")}>
      <div
        className={cn(
      ...
      >
        ...
        <p className={cn("mt-1 text-[10px]", fromCustomer || fromSystem ? "text-[#8a7967]" : "text-white/70")}>
